...
function print_help_menu() {
    ...
    ###########################################################################
                  This is the help menu for SequelTools version 1.3.3
    ###########################################################################
    ...
    General arguments:
        ...
        -v : A verbose option for those who want updates as the program 
             progresses
        -h : Opens this help menu

        Quality Control tool arguments:
        -k : Keep intermediate files (these are removed by default)
        -g : Groups desired (only works when scraps files are included). 
             Options: 'a' for all (CLRs, subedCLRs, subreads, and longest 
             subreads), and 'b' for basic (subedCLRs and subreads). Default is 
             'a'.
        -p : Plots desired.  Options: 'b' for basic (N50 barplot, summary data 
             table, and total bases barplot), 'i' for intermediate (N50 barplot,
             summary data table, total bases barplot, ZOR plot, PSR plot, 
             boxplot of subread read lengths with N50, boxplot of subedCLR read 
             lengths with N50), and 'a' for all (N50 barplot, L50 barplot, 
             summary data table, read length histograms, total bases barplot, 
             ZOR plot, PSR plot, Boxplot of subread read lengths with N50, 
             boxplot of subedCLR read lengths with N50, subreads/CLR frequency 
             plot, and adapters/CLR frequency plot).  Default is 'i'. The 
             boxplot of subedCLR read lengths with N50, subreads/CLR frequency 
             plot, and adapters/CLR frequency plot are only produced if scraps 
             files are included.
        -r : An R script the user has chosen to use instead of the provided R 
             script for plotting.  Keep in mind that SequelTools uses separate 
             R scripts for running with and without scraps files.
        -s : Skips samtools and Python steps for the purpose of testing an 
             alternative R script.  This argument is only appropriate after 
             having run SequelTools with the -k argument sucussfully in the same
             folder such that the intermediate files generated by samtools and 
             Python have already been generated and have not been deleted at the
             end of SequelTools' operation.

    Read Subsampling & Read Filtering tools arguments
        -f : This option allows the user to choose the format of the results 
             files.  Options: 's' for SAM, 'b' for BAM, and '2' for both.  
             Default is 's' simply because the subsetting longest subreads tool 
             runs faster with this option. 

    Read Subsampling tool arguments
        -T : Choose one or more of the subsampling options: 'l' for longest 
             subreads and 'r' for random CLR subsampling. No default is 
             provided.
        -R : When using the random CLR subsampling option, this argument 
             specifies the proportion of CLRs retained from 0 to 1, where 0 
             means all CLRs are discarded and 1 means all CLRs are retained.  
             Default is 0.1.
